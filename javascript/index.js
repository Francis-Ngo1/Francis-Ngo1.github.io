...
function Hero(game, x, y) {
    ...
    //Prevent the hero character from going off the screen with this boundary.
    this.body.collideWorldBounds = true;

    //Adds the animations for Hero
...
//Controls movement speed of hero.
Hero.prototype.move = function (direction) {
    ...
    //Conditional to make the hero flip direction if moving left or right
...
//Hero Jumping Mechanics
Hero.prototype.jump = function () {
    const JUMP_SPEED = 400;
    //Conditional check to see if if a body is touching another body
...
//Stops the player from jumping high
Hero.prototype.stopHighJump = function () {
    this.highJumping = false;
}

//Adds bounce mechanic
Hero.prototype.bounce = function () {
    const BOUNCE_SPEED = 200;
    this.body.velocity.y = -BOUNCE_SPEED;
}

//One Shot One Kill, The hero gets hit once, they dead!
//Function controls LIFE and DEATH animations of hero, actually only dying.
Hero.prototype.die = function () {
  // his removes the sprite from physics operations, so its not taken into account for collisions with spider enemies
...
Hero.prototype._getAnimationName = function() {
    ...
    //Falling animation - y velocity is positive; not touching platform
...
Hero.prototype.update = function () {
    // u=Update sprite animation if needed
...
function Spider(game, x, y) {
    ...
    //anchor
    this.anchor.set(0.5);
    //Animations for spider sprite sheet
...
// Spider update function gets called each frame
...
Spider.prototype.die = function () {
    // This removes the sprite from physics operations, so its not taken into account for collisions.
    // No dead spiders in MY HOUSE
    this.body.enable = false;
    this.animations.play('die').onComplete.addOnce(function () {
        this.kill();
    }, this);
...
//PlayState class
//Used to restart game when levels complete
const LEVEL_COUNT = 2; 
PlayState = {};

PlayState.init = function (data) {
     // This rounds pixels for smoother movements
    this.game.renderer.renderSession.roundPixels = true;
    // Controls user key input for controls and movement
...
PlayState.preload = function () {
    ...
    // LOAD KEY (keymaster and gatekeeper)
    this.game.load.image('key', 'images/key.png');

    // LOAD sprite sheets (animated images)
...
// This gets called each frame so that there is responsive user input controls and real time death
...
//Display Text onscreen for player death "OUCH!"
...
PlayState._spawnPlatform = function (platform) {
    ...
    // Create invisible walls for enemies so they don't go all over the damn place
...
//Spawns those invisible walls to contain enemies
...
window.onload = function () {
    ...
    // true - keep cache
    // false - don't key existing world objects
